...
function drawRemIris()
  ...
  elseif sg.remoteAddress()~="" then
    ...
    if not(netIrisState[sg.remoteAddress()]) then
      temp=0
    elseif netIrisState[sg.remoteAddress()]=="Closed" or netIrisState[sg.remoteAddress()]=="Closing" or netIrisState[sg.remoteAddress()]=="Opening" then
      temp=-1
    else
      temp=1
    end
...
while true do
  ...
  elseif event == "sgStargateStateChange" then
    ...
    drawHome()
    status, int = sg.stargateState()
  elseif event == "sgIrisStateChange" and (param2=="Closed" or param2=="Open") then
    while not(eChest.getStackInSlot(1)) do
      sleep(1)
    end
    eChest.pushItem('up',1,1)
